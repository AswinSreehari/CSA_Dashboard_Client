...
export let FilterType = ((function(FilterType) {
  FilterType["STATUS"] = "Status";
  FilterType["ASSIGNEE"] = "Assignee";
  FilterType["LABELS"] = "Labels";
  FilterType["PRIORITY"] = "Priority";
  FilterType["DUE_DATE"] = "Due date";
  FilterType["CREATED_DATE"] = "Created date";
  FilterType["UPDATED_DATE"] = "Updated date";
  return FilterType;
})({}));

export let FilterOperator = ((function(FilterOperator) {
  FilterOperator["IS"] = "is";
  FilterOperator["IS_NOT"] = "is not";
  FilterOperator["IS_ANY_OF"] = "is any of";
  FilterOperator["INCLUDE"] = "include";
  FilterOperator["DO_NOT_INCLUDE"] = "do not include";
  FilterOperator["INCLUDE_ALL_OF"] = "include all of";
  FilterOperator["INCLUDE_ANY_OF"] = "include any of";
  FilterOperator["EXCLUDE_ALL_OF"] = "exclude all of";
  FilterOperator["EXCLUDE_IF_ANY_OF"] = "exclude if any of";
...
export let Assignee = ((function(Assignee) {
  Assignee["ANDREW_LUO"] = "Andrew Luo";
  Assignee["NO_ASSIGNEE"] = "No assignee";
  return Assignee;
})({}));

export let Labels = ((function(Labels) {
  Labels["BUG"] = "Bug";
...
export let Priority = ((function(Priority) {
...
export let DueDate = ((function(DueDate) {
  DueDate["IN_THE_PAST"] = "in the past";
  DueDate["IN_24_HOURS"] = "24 hours from now";
  DueDate["IN_3_DAYS"] = "3 days from now";
  DueDate["IN_1_WEEK"] = "1 week from now";
  DueDate["IN_1_MONTH"] = "1 month from now";
  DueDate["IN_3_MONTHS"] = "3 months from now";
  return DueDate;
})({}));
...
export const statusFilterOptions = Object.values(Status).map((status) => ({
  name: status,
  icon: <FilterIcon type={status} />,
}));

export const assigneeFilterOptions = Object.values(Assignee).map((assignee) => ({
  name: assignee,
  icon: <FilterIcon type={assignee} />,
}));

export const labelFilterOptions = Object.values(Labels).map((label) => ({
  name: label,
  icon: <FilterIcon type={label} />,
}));

export const priorityFilterOptions = Object.values(Priority).map((priority) => ({
  name: priority,
  icon: <FilterIcon type={priority} />,
}));

export const dateFilterOptions = Object.values(DueDate).map((date) => ({
  name: date,
  icon: undefined,
}));
...
const FilterValueCombobox = ({
  ...
  const nonSelectedFilterValues = filterViewToFilterOptions[filterType]?.filter((filter) => !filterValues.includes(filter.name));
  return (
    <Popover
      open={open}
      onOpenChange={(open) => {
              ...
              <CommandGroup>
                {filterViewToFilterOptions[filterType].map((filter) => (
                  <CommandItem
                    ...
                    <Check
                      className={cn("ml-auto", filterValues.includes(filter.name)
                        ? "opacity-100"
                        : "opacity-0")} />
                  </CommandItem>
                ))}
      ...
      {filters
        ...
        .map((filter) => (
          <div key={filter.id} className="flex gap-[1px] items-center text-xs">
            <div
              className="flex gap-1.5 shrink-0 rounded-l bg-muted px-1.5 py-1 items-center">
